...
func TestHttpDo(t *testing.T) {
	...
	for tName, tc := range testCases {
		...
		r.NoError(err, tName)
		r.Equal(res.Returns.Body, tc.expected.Body, tName)
		r.Equal(res.Returns.StatusCode, tc.expected.StatusCode, tName)
	}

	// test ratelimiter
	rateLimiter = ratelimiter.NewRateLimiter(ctx, 1)
